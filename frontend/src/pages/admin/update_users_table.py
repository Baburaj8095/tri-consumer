...
path = r"C:\Users\Baburaj\Desktop\Trikonekt\trikonekt-marketing\tri-consumer\frontend\src\pages\admin\AdminDashboard.jsx"

with open(path, "r", encoding="utf-8") as f:
    content = f.read()

# We want to replace from {/* Table wrapping */} up to the end of UsersTable return
search_start = "      {/* Table wrapping */}"
search_end = "  );\n}\n\nfunction AdminsTable"

# Get the desktop table code
...
desktop_chunk = content[start_idx:end_idx]

# Extract the table wrap
table_start = desktop_chunk.find("<div className=\"admin-card-table-wrap\">")
pagination_start = desktop_chunk.find("{/* Pagination controls */}")

table_code = desktop_chunk[table_start:pagination_start].strip()

replacement = """      {/* Table wrapping */}
      {!isMobile ? (
""" + "        " + table_code.replace("\n", "\n        ") + """
      ) : (
...
print("Replaced UsersTable successfully")
